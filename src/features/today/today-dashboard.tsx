        ...
        <div className="soft-flow soft-task-cards grid gap-3 sm:grid-cols-2">
          {habits.map((habit) => (
            <article className="group relative grid min-h-40 grid-cols-[58px_1fr_auto] items-center gap-5 border border-white/50 p-6 sm:min-h-44 sm:grid-cols-[68px_1fr_auto] sm:gap-6 sm:p-7 xl:min-h-52 xl:grid-cols-[78px_1fr_auto] xl:pl-14 xl:pr-9 2xl:pl-16" key={habit.id}>
              ...
              <div className="min-w-0"><h2 className={`text-2xl font-bold tracking-[-0.035em] sm:text-3xl xl:text-4xl ${habit.status === "complete" ? "text-[var(--soft-muted)] line-through decoration-[var(--soft-muted)]/70" : ""}`}>{habit.name}</h2><p className="mt-2 text-sm font-medium text-[var(--soft-muted)] sm:text-base xl:text-lg">{habit.category} · {habit.target}</p></div>
