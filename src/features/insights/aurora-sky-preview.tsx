    ...
    <div className={`relative flex h-full flex-col ${isStory ? "px-5 pb-6 pt-7" : "p-7"}`}>
      ...
      <section className={`${isStory ? "mt-7" : "mt-5 grid grid-cols-[.72fr_1.28fr] items-center gap-5"}`}>
        <div className={isStory ? "text-center" : ""}>
          ...
          <p className="mt-3 text-[8px] font-black uppercase tracking-[.2em] text-white/78">rhythm glow</p>
          <p className={`${isStory ? "mx-auto mt-4 max-w-[190px]" : "mt-4 max-w-[155px]"} text-[10px] font-medium leading-4 text-white/82`}>Your month left a calm signal in the sky.</p>
        </div>
        ...
        <div aria-label={`${habits.length} habit auroras`} className={`relative overflow-hidden rounded-[24px] border border-white/10 bg-black/10 shadow-[inset_0_0_30px_rgba(0,0,0,.18)] ${isStory ? "mt-7 h-52" : "h-48"}`}>
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border border-white/20 bg-[#07131d]/70 px-3 py-1.5 text-[7px] font-bold uppercase tracking-[.13em] text-white/82 backdrop-blur-md">Each ribbon is a daily ritual</div>
        </div>
      </section>
      ...
      <footer className="mt-auto flex items-end justify-between text-[7px] font-semibold uppercase tracking-[.14em] text-white/62"><span>A month written in light.</span><span className="text-xl font-black tracking-[-.08em] text-white/90">A.</span></footer>
    </div>
  </article>;
}
