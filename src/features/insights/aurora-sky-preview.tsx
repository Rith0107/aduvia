...
export function AuroraSkyPreview({ completedQuests, consistency, daysShownUp, format, habits, monthName, year }: AuroraSkyPreviewProps) {
  ...
  <article aria-label={rasterUrl ? undefined : "Aurora Sky share preview"} className="absolute inset-0 overflow-hidden rounded-[26px] border border-white/25 text-white shadow-[0_28px_70px_rgba(3,10,18,.42),inset_0_0_0_1px_rgba(255,255,255,.06)]" ref={artworkRef} style={{ backgroundImage: "linear-gradient(155deg,var(--chart-deep) 0%,var(--chart-blue) 58%,var(--chart-deep) 100%)" }}>
    <div className="absolute inset-0 bg-[linear-gradient(155deg,rgba(2,8,15,.22),rgba(2,8,15,.58))]" />
    ...
    <div className="absolute -inset-x-[12%] top-[36%] h-[10%] -rotate-[4deg] rounded-[50%] opacity-30" style={{ backgroundImage: "linear-gradient(90deg,var(--chart-green) 0%,var(--chart-green) 18%,var(--chart-primary) 52%,var(--chart-blue) 82%,var(--chart-blue) 100%)" }} />
    <div className="absolute -inset-x-[12%] top-[41%] h-[7%] rotate-[5deg] rounded-[50%] opacity-24" style={{ backgroundImage: "linear-gradient(90deg,var(--chart-blue) 0%,var(--chart-green) 28%,var(--chart-rust) 68%,var(--chart-primary) 100%)" }} />
    ...
    <div className={`relative flex h-full flex-col ${isStory ? "px-5 pb-10 pt-5" : "px-7 pb-14 pt-7"}`}>
      ...
      <section className={`${isStory ? "mt-10" : "mt-5 grid grid-cols-[.72fr_1.28fr] items-center gap-5"}`}>
        ...
        <div aria-label={`${habits.length} habit auroras`} className={`relative ${isStory ? "mt-1 h-48 overflow-visible" : "h-48 overflow-hidden rounded-[24px] border border-white/10 bg-black/10 shadow-[inset_0_0_30px_rgba(0,0,0,.18)]"}`}>
          <div className="absolute inset-[5%] opacity-55" style={{ backgroundImage: "radial-gradient(ellipse 68% 22% at 30% 36%,var(--chart-green) 0%,transparent 72%),radial-gradient(ellipse 64% 22% at 70% 58%,var(--chart-primary) 0%,transparent 74%),radial-gradient(ellipse 52% 24% at 76% 32%,var(--chart-blue) 0%,transparent 72%)" }} />
          <div className="absolute left-1/2 top-1/2 size-24 -translate-x-1/2 -translate-y-1/2 rounded-full p-[3px] shadow-[0_0_18px_var(--chart-primary),0_0_42px_var(--chart-primary)]" style={{ background: `conic-gradient(var(--chart-primary) ${consistency * 3.6}deg,rgba(255,255,255,.13) 0deg)` }}><div className="grid size-full place-items-center rounded-full border border-white/12 bg-[var(--chart-deep)]"><span className="size-2.5 rounded-full bg-white shadow-[0_0_8px_white,0_0_20px_var(--chart-primary)]" /></div></div>
