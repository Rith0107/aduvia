...
export function AuroraSkyPreview({ completedQuests, consistency, daysShownUp, format, habits, monthName, onRasterReadyChange, year }: AuroraSkyPreviewProps) {
  ...
  <article aria-label={rasterUrl ? undefined : "Aurora Sky share preview"} className="absolute inset-0 overflow-hidden rounded-[26px] border border-white/25 bg-[linear-gradient(155deg,color-mix(in_srgb,var(--chart-deep)_78%,#07131d)_0%,color-mix(in_srgb,var(--chart-deep)_72%,#081827)_54%,color-mix(in_srgb,var(--chart-deep)_46%,#09111f)_100%)] text-white shadow-[0_28px_70px_rgba(3,10,18,.42),inset_0_0_0_1px_rgba(255,255,255,.06)]" ref={artworkRef} style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
    ...
    <div className={`absolute -inset-x-[12%] h-[32%] opacity-80 ${isStory ? "top-1/2 -translate-y-1/2" : "top-[27%]"}`} style={{ backgroundImage: "radial-gradient(ellipse 128% 42% at 50% 52%,color-mix(in srgb,var(--chart-primary) 34%,transparent) 0%,color-mix(in srgb,var(--chart-primary) 15%,transparent) 48%,transparent 82%),radial-gradient(ellipse 92% 36% at 24% 44%,color-mix(in srgb,var(--chart-green) 40%,transparent) 0%,color-mix(in srgb,var(--chart-green) 16%,transparent) 48%,transparent 78%),radial-gradient(ellipse 86% 34% at 78% 60%,color-mix(in srgb,var(--chart-rust) 34%,transparent) 0%,color-mix(in srgb,var(--chart-rust) 12%,transparent) 50%,transparent 78%),radial-gradient(ellipse 72% 32% at 86% 30%,color-mix(in srgb,var(--chart-blue) 30%,transparent) 0%,transparent 76%)" }} />
    ...
    <div className={`relative flex h-full flex-col ${isStory ? "px-5 pb-10 pt-5" : "px-7 pb-14 pt-7"}`}>
      ...
      <section className={`${isStory ? "absolute inset-x-5 bottom-[72px] h-[60px] items-center py-1.5" : "mt-4 py-3"} grid grid-cols-3 border-y border-white/18 text-center`}>
      ...
      {isStory && completedQuests.length > 0 && <section aria-label="Completed quest symbols" className="absolute inset-x-5 bottom-9 flex h-9 items-center justify-center gap-6 text-[color-mix(in_srgb,var(--chart-primary)_70%,white)]">
      ...
      <footer className={`${isStory ? "absolute inset-x-5 bottom-3 h-6" : "absolute inset-x-7 bottom-4"} flex items-end justify-between border-t border-white/16 pt-2 text-[7px] font-semibold uppercase tracking-[.14em] text-white/72`}><span>A month written in light.</span><span className="text-xl font-black tracking-[-.08em] text-white/95">A.</span></footer>
    </div>
  </article>
