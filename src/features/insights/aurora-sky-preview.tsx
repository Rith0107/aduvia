...
export function AuroraSkyPreview({ completedQuests, consistency, daysShownUp, format, habits, monthName, year }: AuroraSkyPreviewProps) {
  ...
  return <article aria-label="Aurora Sky share preview" className={`${shell} relative overflow-hidden rounded-[26px] border border-white/25 bg-[linear-gradient(155deg,#07131d_0%,color-mix(in_srgb,var(--chart-deep)_72%,#081827)_54%,#09111f_100%)] text-white shadow-[0_28px_70px_rgba(3,10,18,.42),inset_0_0_0_1px_rgba(255,255,255,.06)]`}>
    ...
    <div className={`relative flex h-full flex-col ${isStory ? "px-5 pb-12 pt-7" : "px-7 pb-14 pt-7"}`}>
      ...
      <section className={`${isStory ? "mt-4" : "mt-5 grid grid-cols-[.72fr_1.28fr] items-center gap-5"}`}>
        <div className={isStory ? "text-center" : ""}>
          <p className={`${isStory ? "text-[4.5rem]" : "text-[5.5rem]"} font-semibold leading-[.82] tracking-[-.09em]`}>{consistency}<span className="ml-1 text-2xl text-[var(--chart-primary)]">%</span></p>
          <p className={`${isStory ? "mt-2" : "mt-3"} text-[8px] font-black uppercase tracking-[.2em] text-white/78`}>rhythm glow</p>
          <p className={`${isStory ? "mx-auto mt-2 max-w-[210px] text-[9px]" : "mt-4 max-w-[155px] text-[10px]"} font-medium leading-4 text-white/82`}>Your month left a calm signal in the sky.</p>
        </div>

        <div aria-label={`${habits.length} habit auroras`} className={`relative overflow-hidden rounded-[24px] border border-white/10 bg-black/10 shadow-[inset_0_0_30px_rgba(0,0,0,.18)] ${isStory ? "mt-4 h-40" : "h-48"}`}>
      ...
      <section className={`${isStory ? "mt-3 py-2" : "mt-4 py-3"} grid grid-cols-3 border-y border-white/18 text-center`}>
      ...
      <section className={`${isStory ? "mt-4" : "mt-5"}`}>
