      ...
      <div className="insights-flow">
          <section className="mt-12 grid gap-4 xl:grid-cols-[1.35fr_0.75fr]">
            <article className="relative overflow-hidden rounded-[28px] bg-[#143d31] p-6 text-white shadow-[0_22px_55px_rgba(20,61,49,0.2)] sm:p-8">
              ...
              <p aria-live="polite" className="relative mt-7 text-[10px] font-semibold uppercase tracking-[0.15em] text-white/45">{activeTrend ? `Week of ${activeTrend.label} · ${activeTrend.score}% consistency` : "Weekly consistency trend · hover for details"}</p>
            ...
            <article className="rounded-[28px] border border-[#174f3a]/10 bg-[#fffaf0] p-6 sm:p-7">
              <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-400">Life balance</p><h2 className="mt-2 text-xl font-semibold tracking-[-0.03em]">Where your effort went</h2><p aria-live="polite" className="mt-2 text-xs font-semibold text-[var(--soft-accent)]">{activeCategory ? `${activeCategory.name} · ${activeCategory.value}% of effort` : "Hover a segment for its share"}</p></div>
              <div className="relative mx-auto mt-3 h-56 max-w-[280px]">
                <ResponsiveContainer height="100%" width="100%"><PieChart><Pie cx="50%" cy="50%" data={categoryBalance} dataKey="value" innerRadius={62} nameKey="name" onMouseEnter={(_, index) => setActiveCategory(categoryBalance[index])} onMouseLeave={() => setActiveCategory(null)} outerRadius={91} paddingAngle={4} stroke="none">{categoryBalance.map((entry) => <Cell fill={entry.color} key={entry.name} />)}</Pie></PieChart></ResponsiveContainer>
          ...
          <section className="mt-4 grid gap-4 lg:grid-cols-[0.72fr_1.28fr]">
            ...
            <article className="rounded-[24px] bg-[#f3e7ca] p-5 text-[#6e5b3c] sm:p-6"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] opacity-55">Weekly rhythm</p><p className="mt-2 text-lg font-semibold">Completion by weekday</p></div><p aria-live="polite" className="text-right text-xs font-semibold opacity-65">{activeWeekday ? `${activeWeekday.name} · ${activeWeekday.score}%` : "Last 30 days · hover each bar"}</p></div><div className="mt-4 h-28"><ResponsiveContainer height="100%" width="100%"><BarChart data={weekdayRhythm}><XAxis axisLine={false} dataKey="day" tick={{ fill: "#876f47", fontSize: 10 }} tickLine={false} /><Bar dataKey="score" fill="#876f47" onMouseEnter={(_, index) => setActiveWeekday(weekdayRhythm[index])} onMouseLeave={() => setActiveWeekday(null)} radius={[7, 7, 2, 2]} /></BarChart></ResponsiveContainer></div></article>
          </section>
