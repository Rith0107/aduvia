            ...
            <div className="soft-flow soft-task-cards grid gap-3">
              {visibleHabits.map((habit) => (
                <article className="grid min-h-32 gap-4 border border-white/50 p-5 sm:grid-cols-[minmax(0,1.4fr)_170px_110px_100px] sm:items-center sm:p-6" key={habit.id}>
                  <div className="flex items-center gap-4"><span className="grid size-14 shrink-0 place-items-center rounded-full bg-white/45"><ActivityIcon activity={`${habit.name} ${habit.category}`} /></span><div><h3 className="font-bold">{habit.name}</h3><p className="text-sm text-[var(--soft-muted)]">{habit.category} · {habit.frequency}</p></div></div>
