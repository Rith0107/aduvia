              ...
              {visibleHabits.map((habit) => {
                ...
                return (
                  <article className={`habit-library-card ${expandedHabitId === habit.id ? "is-expanded" : ""} grid min-h-36 gap-5 border border-white/50 p-5 sm:p-6 xl:grid-cols-[minmax(220px,.9fr)_minmax(250px,1.05fr)_145px_minmax(165px,.68fr)_76px_104px] xl:items-center xl:gap-x-8 xl:px-8 2xl:grid-cols-[minmax(250px,.9fr)_minmax(280px,1.08fr)_155px_minmax(185px,.72fr)_84px_110px] 2xl:gap-x-10`} key={habit.id}>
                    ...
                    <button aria-expanded={expandedHabitId === habit.id} className="habit-mobile-summary" onClick={() => setExpandedHabitId((current) => current === habit.id ? null : habit.id)} type="button"><span className={`habit-status-dot ${healthTone}`} /><span><small>Status</small><strong>{health.label}</strong></span><span><small>This month</small><strong>{displayedConsistency(habit)}%</strong></span><span><small>Streak</small><strong>{habit.streak} days</strong></span><ChevronDown aria-hidden /></button>
