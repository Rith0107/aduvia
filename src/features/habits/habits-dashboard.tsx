              ...
              {visibleHabits.map((habit) => {
                ...
                return (
                  <article className="grid min-h-36 gap-5 border border-white/50 p-5 sm:p-6 xl:grid-cols-[minmax(220px,.9fr)_minmax(250px,1.05fr)_145px_minmax(165px,.68fr)_76px_104px] xl:items-center xl:gap-x-8 xl:px-8 2xl:grid-cols-[minmax(250px,.9fr)_minmax(280px,1.08fr)_155px_minmax(185px,.72fr)_84px_110px] 2xl:gap-x-10" key={habit.id}>
                    ...
                    <div className="xl:pl-2"><p className="text-[10px] font-bold uppercase tracking-[.13em] text-[var(--soft-muted)]">Status</p><span className={`mt-2.5 inline-flex rounded-full border px-2.5 py-1.5 text-[9px] font-black uppercase tracking-[.1em] ${healthTone}`}>{health.label}</span></div>
                    <div className="xl:border-l xl:border-black/[0.06] xl:pl-6"><p className="text-xs text-[var(--soft-muted)]">This month</p><div className="mt-2 flex items-center gap-3"><div className="h-2 min-w-0 flex-1 overflow-hidden rounded-full bg-white/55"><div className="h-full rounded-full bg-[var(--soft-icon-green)]" style={{ width: `${displayedConsistency(habit)}%` }} /></div><span className="shrink-0 text-xs font-semibold">{displayedConsistency(habit)}%</span></div></div>
                    <div className="xl:pl-1"><p className="text-xs text-stone-400">Streak</p><p className="mt-1 whitespace-nowrap text-base font-semibold">{habit.streak} days</p></div>
