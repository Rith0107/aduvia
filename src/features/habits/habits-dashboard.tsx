          ...
          <section className="mt-10 border-t border-black/[0.09] pt-7">
            ...
            <div className="soft-flow soft-task-cards grid gap-3">
              {visibleHabits.map((habit) => {
                ...
                return (
                  <article className="grid min-h-36 gap-5 border border-white/50 p-5 sm:p-6 lg:grid-cols-[minmax(230px,.85fr)_minmax(250px,1fr)_145px_170px_80px_100px] lg:items-center xl:grid-cols-[minmax(280px,.9fr)_minmax(310px,1.1fr)_155px_190px_90px_110px] xl:px-8" key={habit.id}>
                    ...
                    <div><p className="text-[10px] font-bold uppercase tracking-[.13em] text-[var(--soft-muted)]">Status</p><span className={`mt-2.5 inline-flex rounded-full border px-2.5 py-1.5 text-[9px] font-black uppercase tracking-[.1em] ${healthTone}`}>{health.label}</span></div>
