...
import { ChartNoAxesColumnIncreasing, ChevronDown, Flag, Flame, Gauge, ListChecks, Pencil } from "lucide-react";
...
export function HabitsDashboard({ initialHabits, previewMode = false }: HabitsDashboardProps) {
  ...
  return (
    <AppShell active="Habits" eyebrow="Build your rhythm" screenClassName="habits-screen" title={<>Habits that feel<br />like your own.</>} action={<button className="habits-new-button rounded-full bg-[var(--soft-ink)] px-6 py-4 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5" onClick={openCreateHabit} type="button">+ New habit</button>}>
          <section className="habits-summary mt-12 grid border-y border-black/[0.09] sm:grid-cols-2 xl:grid-cols-[.7fr_1fr_1fr_1fr]">
          ...
          <section className="habits-library mt-10 border-t border-black/[0.09] pt-7">
            <div className="habits-library-header flex flex-col gap-4 pb-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="habits-filters flex rounded-full bg-white/45 p-1">
            ...
            <div className="habits-card-list soft-flow soft-task-cards grid gap-3">
              {visibleHabits.map((habit) => {
                ...
                const healthTone = {
                  strong: "border-[#b8d8ca] bg-[#dcebe4] text-[#216b55]",
                  steady: "border-[#bdd5df] bg-[#dceaf0] text-[#2f6175]",
                  reset: "border-[#e8c5b2] bg-[#f4dfd0] text-[#8a4636]",
                  learning: "border-[#d7d6d0] bg-[#ecebe6] text-[#585c57]",
                  paused: "border-[#cfcfca] bg-[#e3e3df] text-[#5d615c]",
                  completed: "border-[#dacb91] bg-[#eee3bd] text-[#65551f]",
                }[health.tone];
                return (
                  <article className={`habit-library-card ${expandedHabitId === habit.id ? "is-expanded" : ""} grid min-h-36 gap-5 border border-white/50 p-5 sm:p-6 xl:grid-cols-[minmax(220px,.9fr)_minmax(250px,1.05fr)_145px_minmax(165px,.68fr)_76px_104px] xl:items-center xl:gap-x-8 xl:px-8 2xl:grid-cols-[minmax(250px,.9fr)_minmax(280px,1.08fr)_155px_minmax(185px,.72fr)_84px_110px] 2xl:gap-x-10`} key={habit.id}>
                    <div className="habit-identity flex items-center gap-5"><span className="grid size-12 shrink-0 place-items-center"><ActivityIcon activity={`${habit.name} ${habit.category}`} className="size-8" /></span><div><div className="flex flex-wrap items-center gap-2"><h3 className={`text-lg font-bold tracking-[-0.025em] ${habit.state === "completed" ? "text-[var(--soft-muted)] line-through" : ""}`}>{habit.name}</h3>{habit.isAnchor && <span className="habit-anchor inline-flex items-center gap-1 rounded-full bg-[var(--soft-tint-b)] px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em] text-[var(--soft-icon-clay)]"><Flag size={10} fill="currentColor" />Anchor</span>}</div><p className="mt-1 text-sm text-[var(--soft-muted)]">{habit.category} · {habit.scheduledDays?.length ? `${habit.frequency === "3× weekly" ? "3× · " : ""}${habit.scheduledDays.join(" · ")}` : habit.frequency}</p></div></div>
                    <span className="habit-mobile-efficiency"><small>This month</small><strong>{displayedConsistency(habit)}%</strong></span>
                    <div className="habit-cadence"><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-[var(--soft-muted)]">Weekly cadence</p><div className="mt-2.5 grid max-w-sm grid-cols-7 gap-1.5">{days.map((day) => { const isScheduled = scheduled.includes(day.short); return <span aria-label={`${day.label}: ${isScheduled ? "scheduled" : "rest day"}`} className={`grid aspect-square max-w-9 place-items-center rounded-[10px] text-[10px] font-bold ${isScheduled ? "bg-[var(--soft-icon-green)] text-white" : "bg-white/45 text-[var(--soft-muted)] opacity-45"}`} key={day.short}>{day.short.slice(0, 2)}</span>; })}</div></div>
                    <button aria-expanded={expandedHabitId === habit.id} aria-label={`${expandedHabitId === habit.id ? "Hide" : "Show"} details for ${habit.name}`} className="habit-mobile-summary" onClick={() => setExpandedHabitId((current) => current === habit.id ? null : habit.id)} type="button"><span className={`habit-status-dot ${healthTone}`} /><span><small>Status</small><strong>{health.label}</strong></span><span><small>This month</small><strong>{displayedConsistency(habit)}%</strong></span><span><small>Streak</small><strong>{habit.streak} days</strong></span><ChevronDown aria-hidden /></button>
                    <div className="habit-health xl:pl-2"><p className="text-[10px] font-bold uppercase tracking-[.13em] text-[var(--soft-muted)]">Status</p><span className={`mt-2.5 inline-flex rounded-full border px-2.5 py-1.5 text-[9px] font-black uppercase tracking-[.1em] ${healthTone}`}>{health.label}</span></div>
                    <div className="habit-progress xl:border-l xl:border-black/[0.06] xl:pl-6"><p className="text-xs text-[var(--soft-muted)]">This month</p><div className="mt-2 flex items-center gap-3"><div className="h-2 min-w-0 flex-1 overflow-hidden rounded-full bg-white/55"><div className="h-full rounded-full bg-[var(--soft-icon-green)]" style={{ width: `${displayedConsistency(habit)}%` }} /></div><span className="shrink-0 text-xs font-semibold">{displayedConsistency(habit)}%</span></div></div>
                    <div className="habit-streak xl:pl-1"><p className="text-xs text-stone-400">Streak</p><p className="mt-1 whitespace-nowrap text-base font-semibold">{habit.streak} days</p></div>
                    <div className="habit-actions grid gap-2"><button aria-label={`Edit ${habit.name}`} className="inline-flex items-center justify-center gap-1.5 rounded-full border border-[var(--soft-icon-green)]/20 bg-white/70 px-3 py-2.5 text-xs font-bold text-[var(--soft-icon-green)] shadow-sm transition hover:-translate-y-0.5 hover:bg-white" onClick={() => openEditHabit(habit)} type="button"><Pencil aria-hidden className="size-3" />Edit habit</button><button className={`rounded-full px-3 py-2.5 text-xs font-bold transition ${habit.state === "active" ? "bg-white/55" : "bg-[var(--soft-ink)] text-white"}`} onClick={() => toggleState(habit.id)} type="button">{habit.state === "active" ? "Pause" : "Resume"}</button></div>
                  </article>
                );
              })}
            </div>
