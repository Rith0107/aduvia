...
export function DesignLab() {
  ...
  return (
    <main className="min-h-screen bg-[#ebe9e3] p-3 text-[#17201c] sm:p-6">
      ...
      <section aria-label="Aduvia colour schemes" className="mx-auto mb-6 grid max-w-[1800px] gap-3 md:grid-cols-3">
        {(Object.entries(softPalettes) as Array<[SoftPalette, (typeof softPalettes)[SoftPalette]]>).map(([id, palette]) => {
          const selected = direction === "soft" && softPalette === id;
          const swatches = [{ label: "Canvas", color: palette.base }, { label: "Ink", color: palette.ink }, { label: "Accent", color: palette.accent }, { label: "Glow A", color: palette.glowA }, { label: "Glow B", color: palette.glowB }, { label: "Surface", color: palette.cards[0] }];
          return <button aria-pressed={selected} className={`overflow-hidden rounded-[24px] border bg-white/65 p-3 text-left transition hover:-translate-y-0.5 ${selected ? "border-[#17201c] shadow-[0_14px_35px_-24px_rgba(23,32,28,.55)]" : "border-white/70"}`} key={id} onClick={() => { setDirection("soft"); setSoftPalette(id); }} type="button"><div className="grid h-16 grid-cols-6 overflow-hidden rounded-[16px]">{swatches.map((swatch) => <span aria-label={`${swatch.label}: ${swatch.color}`} key={swatch.label} style={{ backgroundColor: swatch.color }} />)}</div><div className="flex items-center justify-between px-2 pb-1 pt-3"><div><p className="text-sm font-bold">{palette.label}</p><p className="mt-0.5 text-[10px] text-stone-400">Canvas · ink · accent · glows · surface</p></div><span className={`grid size-6 place-items-center rounded-full text-xs ${selected ? "bg-[#17201c] text-white" : "bg-stone-100 text-transparent"}`}>✓</span></div></button>;
        })}
      </section>
