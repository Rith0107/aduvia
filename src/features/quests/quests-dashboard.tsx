...
type QuestsDashboardProps = { initialQuests: QuestSummary[]; previewMode?: boolean };
...
export function QuestsDashboard({ initialQuests, previewMode = false }: QuestsDashboardProps) {
  ...
  return (
    <AppShell active="Quests" eyebrow={`${monthContext.monthName} · ${monthPhase.label} · ${monthContext.countdownLabel}`} screenClassName="quests-screen" title={<>{monthPhase.questHeadline}</>} action={<div className="quests-page-action flex max-w-sm flex-col items-start gap-4 sm:items-end"><p className="text-left text-sm leading-6 text-[var(--soft-muted)] sm:text-right">{monthPhase.questPrompt}</p><button className="quests-new-button rounded-full bg-[var(--soft-ink)] px-6 py-4 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5" onClick={() => openQuestCreator()} type="button">+ New quest</button></div>}>
          <section className="quests-summary mt-12 grid border-y border-black/[0.09] sm:grid-cols-2 xl:grid-cols-[.8fr_.8fr_1.2fr_1.2fr]">
          ...
          <section className="quests-board mt-10 border-t border-black/[0.09] pt-7">
            <div className="quests-board-header flex flex-col gap-4 pb-5 sm:flex-row sm:items-center sm:justify-between">
              <div>
              ...
              <div className="quests-filters flex max-w-full flex-wrap gap-1 overflow-x-auto rounded-full bg-white/45 p-1">
