                    ...
                    return <article className="overflow-hidden rounded-[34px] border border-white/65 bg-white/30 shadow-[0_24px_70px_-52px_rgba(25,47,38,.65)]" key={month.monthKey}>
                      ...
                      <div className="grid gap-px bg-black/[0.07] sm:grid-cols-2">
                        {month.quests.map((quest, index) => {
                          ...
                          return <div className="group relative min-h-32 bg-[color:color-mix(in_srgb,var(--soft-surface)_86%,transparent)] p-5 transition hover:bg-white/65 sm:p-6" key={quest.id}>
                            <div className="flex items-start justify-between gap-4"><span className={`grid size-10 place-items-center rounded-[14px] ${isComplete ? "bg-[var(--soft-icon-green)] text-white" : "bg-[var(--soft-tint-b)] text-[var(--soft-icon-clay)]"}`}><ActivityIcon activity={`${quest.title} ${quest.category}`} className="size-[18px]" /></span><span className="font-mono text-[10px] font-bold tracking-[.14em] text-[var(--soft-muted)]/55">{String(index + 1).padStart(2, "0")}</span></div>
                            <p className="mt-4 text-[9px] font-black uppercase tracking-[.17em] text-[var(--soft-accent)]">{quest.category}</p>
                            <h4 className="mt-1.5 max-w-md text-lg font-semibold tracking-[-.035em]">{quest.title}</h4>
                            <div className="mt-3 flex items-center gap-2 text-[10px] font-black uppercase tracking-[.1em]"><span className={`grid size-5 place-items-center rounded-full ${isComplete ? "bg-[var(--soft-icon-green)]/15 text-[var(--soft-icon-green)]" : "bg-black/[0.06] text-[var(--soft-muted)]"}`}>{isComplete ? "✓" : "·"}</span><span className={isComplete ? "text-[var(--soft-icon-green)]" : "text-[var(--soft-muted)]"}>{isComplete && quest.completedAt ? `Completed ${new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(new Date(quest.completedAt))}` : "Incomplete at month end"}</span></div>
                          </div>;
                        })}
                      </div>
