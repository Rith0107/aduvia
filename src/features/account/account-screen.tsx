...
import { ArrowLeft, BookOpenText, Database, LogIn, LogOut, ShieldCheck, UserRound } from "lucide-react";
...
export function AccountScreen() {
  ...
  <section className="mt-6 flex flex-col gap-5 rounded-[32px] border border-white/65 bg-[var(--soft-tint-c)]/60 p-7 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-[var(--soft-accent)]">Data & account</p><h2 className="mt-2 text-2xl font-semibold">One home for your progress.</h2><p className="mt-1 text-sm text-[var(--soft-muted)]">Cloud sync will activate when you log in; preview data remains local to this browser.</p></div><Link className="rounded-full bg-white/70 px-5 py-3 text-center text-sm font-bold" href="/insights">View your insights</Link></section></div></div></main>;
}
