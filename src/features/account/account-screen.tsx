...
export function AccountScreen() {
  ...
  <section className="mt-6 flex flex-col gap-5 rounded-[32px] border border-white/65 bg-[var(--soft-tint-c)]/60 p-7 sm:flex-row sm:items-center sm:justify-between" id="privacy"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-[var(--soft-accent)]">Data & account</p><h2 className="mt-2 text-2xl font-semibold">Your progress belongs to you.</h2><p className="mt-1 max-w-2xl text-sm text-[var(--soft-muted)]">{viewer ? "Your habits, quests, check-ins, and private reflections sync securely to your account." : "This preview is stored only in this browser until you create an account."} Download a portable copy whenever you want.</p></div><div className="flex flex-wrap gap-2"><button className="inline-flex items-center gap-2 rounded-full bg-white/70 px-5 py-3 text-center text-sm font-bold disabled:cursor-wait disabled:opacity-45" disabled={!appData || appData.isLoading} onClick={exportData} type="button"><Download className="size-4" />{appData?.isLoading ? "Preparing data…" : "Export my data"}</button><Link className="rounded-full bg-[var(--soft-ink)] px-5 py-3 text-center text-sm font-bold text-white" href="/insights">View insights</Link></div></section>
