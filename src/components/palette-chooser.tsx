...
export function PaletteChooser() {
  ...
  return (
    <div className="relative">
      <button aria-expanded={open} aria-haspopup="menu" aria-label="Choose colour palette" className="grid size-10 place-items-center rounded-full bg-white/45 text-[var(--soft-ink)] shadow-[inset_0_1px_0_rgba(255,255,255,.6)] backdrop-blur-xl transition hover:bg-white/65" onClick={() => setOpen((current) => { if (!current) window.dispatchEvent(new CustomEvent("aduvia:toolbar-menu", { detail: "palette" })); return !current; })} type="button"><Palette className="size-4" strokeWidth={1.8} /></button>
